...
def crear_matriz(nombre_archivo):
	global nX, nY
	matriz = np.zeros((nX, nY))					# Inicializo la matriz
	x, y = 0, 0
	...
	if isinstance(nombre_archivo, str) and os.path.isfile(nombre_archivo):
		...
		matriz = np.random.randint(2, size=(nX, nY))	# Matriz nX x nY aleatoria
...
# Mando a calcular las matrices usando multiprocessing
...
while n <= ITERAC:
	...
	# Calculo la iteracion de la matriz particionada (MP)
